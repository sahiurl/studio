...
import type { AdminPostFormData, FirestorePostData, AdminPostFormQualityOption, AdminPostFormLinkItem, TMDBFormattedSearchResult, FormPopulatableDetails } from '@/types';
import { doc, getDoc, updateDoc, serverTimestamp } from "firebase/firestore";
import { db } from '@/lib/firebase';
import { useToast } from "@/hooks/use-toast";
import { searchTMDBAction, getTMDBDetailsAction } from '../../create/actions';
...
const genreOptions = [
  ...
  { id: 'adult', label: 'Adult' }, { id: '18plus', label: '18+' },
...
export default function EditPostPage() {
    ...
    const fetchPostData = async () => {
      ...
      try {
        ...
        if (postDocSnap.exists()) {
          const postData = postDocSnap.data() as FirestorePostData;
          
          let telegramOptions: AdminPostFormQualityOption[] = postData.telegramOptions || [];
          let directDownloadOptions: AdminPostFormQualityOption[] = postData.directDownloadOptions || [];

          if (postData.downloadOptions && (!postData.telegramOptions || !postData.directDownloadOptions)) {
            telegramOptions = [];
            directDownloadOptions = [];
            postData.downloadOptions.forEach(opt => {
              const telegramLinks: AdminPostFormLinkItem[] = [];
              const directLinks: AdminPostFormLinkItem[] = [];
              ...
              if (telegramLinks.length > 0) {
                telegramOptions.push({ qualityLabel: opt.qualityLabel, links: telegramLinks });
              }
              if (directLinks.length > 0) {
                directDownloadOptions.push({ qualityLabel: opt.qualityLabel, links: directLinks });
              }
            });
          }


          const formData: AdminPostFormData = {
            ...postData,
            languages: Array.isArray(postData.languages) ? postData.languages : [],
            genres: Array.isArray(postData.genres) ? postData.genres : [], 
            rating: postData.rating === null ? undefined : postData.rating,
            runtime: postData.runtime === null ? undefined : postData.runtime,
            totalSeasons: postData.totalSeasons === null ? undefined : postData.totalSeasons,
            totalEpisodes: postData.totalEpisodes === null ? undefined : postData.totalEpisodes,
            backdropUrl: postData.backdropUrl === null ? '' : postData.backdropUrl,
            telegramOptions: telegramOptions.map(opt => ({
              ...opt,
              links: opt.links.map(link => ({ ...link, size: link.size || '' }))
            })),
            directDownloadOptions: directDownloadOptions.map(opt => ({
                ...opt,
                links: opt.links.map(link => ({ ...link, size: link.size || '' }))
            })),
            seoKeywords: Array.isArray(postData.seoKeywords) ? postData.seoKeywords : [],
          };
          reset(formData);
          setCurrentMediaType(formData.mediaType);
        } else {
          toast({
            variant: "destructive",
            title: "Error",
        ...
        toast({
          ...
          description: "Could not load post data for editing.",
        });
      } finally {
  ...
  const populateFormWithTMDBData = (details: FormPopulatableDetails) => {
    ...
    setValue('languages', validLanguages.length > 0 ? validLanguages : (getValues('languages').length > 0 ? getValues('languages') : ['en']), { shouldValidate: true });
    ...
    const matchedGenreIds = genreOptions
      ...
      .map(opt => opt.id);
    setValue('genres', matchedGenreIds.length > 0 ? matchedGenreIds : (getValues('genres').length > 0 ? getValues('genres') : []), { shouldValidate: true });
  ...
  const onSubmit: SubmitHandler<AdminPostFormData> = async (data) => {
    ...
    try {
      ...
      if (postDataToUpdate.mediaType === 'movie') {
        delete (postDataToUpdate as any).totalSeasons;
        delete (postDataToUpdate as any).totalEpisodes;
      } else {
        delete (postDataToUpdate as any).runtime;
      }
      
      delete (postDataToUpdate as any).downloadOptions;


      const postDocRef = doc(db, "posts", postId);
      await updateDoc(postDocRef, postDataToUpdate);
                        ...
                        {languageOptions.map((option) => (
                          <div key={option.id} className="flex items-center space-x-2">
                            <Checkbox
                              ...
                              disabled={isFetchingTMDB}
                            />
                            <Label htmlFor={`lang-edit-${option.id}`} className="font-normal cursor-pointer">
                              {option.label}
                            </Label>
                          </div>
                        ))}
                        ...
                        {genreOptions.map((option) => (
                          <div key={option.id} className="flex items-center space-x-2">
                            <Checkbox
                              ...
                              disabled={isFetchingTMDB}
                            />
                            <Label htmlFor={`genre-edit-${option.id}`} className="font-normal cursor-pointer">
                              {option.label}
                            </Label>
                          </div>
                        ))}
